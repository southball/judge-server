...
export function problemsRouter(): Router {
  ...
  router.post(
    '/problems',
    authAdminMiddleware,
    bodySingleTransformerMiddleware(CreateProblemProps),
    createProblem
  );
  ...
  router.put(
    ...
    authAdminMiddleware,
    bodySingleTransformerMiddleware(EditProblemProps),
    editProblem
  );
  router.delete('/problem/:problem_slug', authAdminMiddleware, deleteProblem);

  router.post(
    '/problem/:problem_slug/submit',
    authUserMiddleware,
    bodySingleTransformerMiddleware(SubmitProblemProps),
    submitProblem
  );

  router.get(
    '/problem/:problem_slug/testcases',
    authAdminMiddleware,
    getTestcases
  );
  router.put(
  ...
  router.get(
    '/problem/:problem_slug/interactor',
    authAdminMiddleware,
    getInteractor
  );
  router.get(
    '/problem/:problem_slug/metadata',
    authAdminMiddleware,
    getMetadata
  );
...
async function getProblems(req: Request, res: Response): Promise<void> {
  ...
  if (req.user?.permissions.includes(Permissions.ADMIN)) {
    ...
    res.json(
      Ok(problems.filter((problem) => problem.is_public).map(toPublicProblem))
    );
  }
}
...
async function createProblem(req: Request, res: Response): Promise<void> {
  ...
  } = req.body as CreateProblemProps;
  ...
  try {
    ...
    if (result.rowCount !== 1) throw new Error('Expected row count to be 1.');
...
async function editProblem(req: Request, res: Response): Promise<void> {
  ...
  problem.compile_time_limit =
    body.compile_time_limit ?? problem.compile_time_limit;
  problem.compile_memory_limit =
    body.compile_memory_limit ?? problem.compile_memory_limit;
  problem.checker_time_limit =
    body.checker_time_limit ?? problem.checker_time_limit;
  problem.checker_memory_limit =
    body.checker_memory_limit ?? problem.checker_memory_limit;
...
async function updateTestcases(req: Request, res: Response): Promise<void> {
  ...
  if (req.is('application/zip') && Buffer.isBuffer(req.body)) {
    ...
    const iterator = streamify(body).pipe(
    ...
    setDifference(inFileSet, outFileSet).forEach((testname) => {
      warnings.push(
        `Input file ${testname} exists but the corresponding output file does not exist.`
      );
      fs.unlinkSync(path.resolve(folder, `${testname}.in`));
    });
    setDifference(outFileSet, inFileSet).forEach((testname) => {
      warnings.push(
        `Output file ${testname} exists but the corresponding input file does not exist.`
      );
    ...
    res.json(
      Err(
        'Only MIME type application/zip and the zip file as body is supported.'
      )
    );
  }
}
...
async function getMetadata(req: Request, res: Response): Promise<void> {
  ...
  res.type('application/x-yaml').end(metadataYaml);
}
