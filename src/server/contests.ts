...
export function contestsRouter(): Router {
  ...
  router.delete('/contest/:contest_slug', authAdminMiddleware, deleteContest);

  router.get(
    '/contest/:contest_slug/problem/:contest_problem_slug',
    getContestProblem
  );
  router.post(
    '/contest/:contest_slug/problem/:contest_problem_slug/submit',
    authUserMiddleware,
...
async function userContestRunningMiddleware(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
 ...
 */
async function fetchContestFromSlug(
  req: Request,
  res: Response,
  next: NextFunction,
  contest_slug: string
...
async function fetchContestProblemFromSlug(
  req: Request,
  res: Response,
  next: NextFunction,
...
async function getContests(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (userIsAdmin(req.user)) {
      ...
      res.json(
        Ok(contests.filter((contest) => contest.is_public).map(toPublicContest))
      );
    }
  } catch (err) {
...
async function getContestRegistrants(
  req: Request,
  res: Response
): Promise<void> {
  const { pool } = AppState.get();
...
class CreateContestProps {
  ...
  @Type(() => CreateContestProblemProps)
  problems: CreateContestProblemProps[] = [];
  @IsDate() @Type(() => Date) start_time: Date = new Date(0);
  @IsDate() @Type(() => Date) end_time: Date = new Date(0);
}

async function validateProblemSlugs(
  problems: CreateContestProblemProps[]
): Promise<void> {
  const { pool } = AppState.get();
  ...
  // Check that the problem slugs are unique.
  {
    const slugSet = new Set([
      ...problems.map((problem) => problem.contest_problem_slug),
    ]);
  ...
  // Check that the problem slugs are valid.
  {
    ...
    if (result.rowCount !== problems.length) {
      const existingSlugs: Set<number> = new Set(
        result.rows.map((problem: Problem) => problem.id)
      );
      const wrongSlugs = problems.filter(
        (slug) => !existingSlugs.has(slug.problem_id)
      );
...
async function createContestProblems(
  client: PoolClient,
  contest: Contest,
  problems: CreateContestProblemProps[]
...
async function createContest(req: Request, res: Response): Promise<void> {
  ...
  const {
    ...
    start_time,
    end_time,
  } = req.body as CreateContestProps;
  ...
  try {
    ...
    if (result.rowCount !== 1) throw new Error('Failed to create contest.');
    const contest = result.rows[0] as Contest;

    const contestProblems = await createContestProblems(
      client,
      contest,
      problems
    );
