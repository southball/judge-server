...
export function adminRouter(): Router {
  ...
  router.get(
    '/admin/submissions',
    authAdminMiddleware,
    bodySingleTransformerMiddleware(GetSubmissionsProps),
    getSubmissions
  );
...
async function getTestlib(req: Request, res: Response): Promise<void> {
  const testlib = fs.readFileSync(
    path.resolve(process.env.DATAFOLDER, 'testlib.h')
  );

  res.type('application/octet-stream').end(testlib);
}
