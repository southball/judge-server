...
import {
  bodySingleTransformerMiddleware,
  querySingleTransformerMiddleware,
} from '../validation';
...
async function getSubmissions(req: Request, res: Response): Promise<void> {
  ...
  const result = await pool.query(
    `SELECT S.*,
             ...
             ORDER BY S.id DESC
             LIMIT $1`,
    [body.count, typeof body.start !== 'number', body.start]
  );
...
async function updateSubmissionJudge(
  req: Request,
  res: Response
): Promise<void> {
  ...
  try {
    ...
    try {
      const progress = body.testcases.filter(
        (testcase) => testcase.verdict !== 'WJ'
      ).length;
...
async function rejudgeSubmission(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    await channel.sendToQueue(
      'JUDGE_QUEUE',
      Buffer.from(req.submission.id.toString())
    );
