...
export type PublicProblem = Pick<
  Problem,
  'id' | 'type' | 'slug' | 'title' | 'statement' | 'time_limit' | 'memory_limit'
>;

export const toPublicProblem = ({
  id,
  type,
  slug,
  title,
...
export type ContestProblemInfo = Problem & {
...
export type PublicContest = Pick<
  Contest,
  'id' | 'slug' | 'title' | 'start_time' | 'end_time'
> & {
  problems?: PublicContestProblemInfo[];
};

export type PublicContestProblemInfo = PublicProblem & {
  contest_problem_slug: string;
  contest_problem_id: number;
};

export const toPublicContest = ({
  id,
  slug,
  title,
  start_time,
...
export type SlimSubmission = Pick<
  RichSubmission,
  | 'id'
  | 'date'
  | 'language'
...
export type PublicSubmission = SlimSubmission &
  Pick<RichSubmission, 'source_code' | 'testcases'>;

export const toSlimSubmission = ({
  id,
  date,
  language,
  verdict,
