...
async function main(): Promise<void> {
  ...
  await fs.promises.mkdir(path.resolve(process.env.DATAFOLDER), {
    recursive: true,
  });
...
async function preInit(): Promise<void> {
  const testlibUrl =
    'https://cdn.jsdelivr.net/gh/MikeMirzayanov/testlib@master/testlib.h';
...
async function initAppState(): Promise<void> {
  ...
  if (typeof key !== 'string' || key.length <= 16) {
    throw new Error(
      'The key is invalid, or it is less than 16 bytes, which is too insecure.'
    );
  }
